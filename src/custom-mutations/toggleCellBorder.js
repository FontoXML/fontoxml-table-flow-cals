define([
	...
	return function toggleCellBorder (argument, blueprint, _format, _selection) {
		...
		var isToggle = Object.keys(borders).reduce(function (numberOfBordersToBeSet, borderKey) {
		...
		if (borders.bottom !== undefined) {
			cellNodeIds.forEach(function (cellNodeId) {
				...
				blueprint.setAttribute(cellInfo.element, 'rowsep', (isToggle ? !bottom : borders.bottom) ? trueValue : falseValue);
			});
		}
		...
		if (borders.right !== undefined) {
			cellNodeIds.forEach(function (cellNodeId) {
				...
				blueprint.setAttribute(cellInfo.element, 'colsep', (isToggle ? !right : borders.right) ? trueValue : falseValue);
			});
		}
		...
		if (borders.top !== undefined) {
			cellNodeIds.forEach(function (cellNodeId) {
				...
				for (var i = 0; i < cellInfo.size.columns; i++) {
					...
					blueprint.setAttribute(neighborCellInfo.element, 'rowsep', (isToggle ? !top : borders.top) ? trueValue : falseValue);
				}
			});
		}
		...
		if (borders.left !== undefined) {
			cellNodeIds.forEach(function (cellNodeId) {
				...
				for (var i = 0; i < cellInfo.size.rows; i++) {
					...
					blueprint.setAttribute(neighborCellInfo.element, 'colsep', (isToggle ? !left : borders.left) ? trueValue : falseValue);
				}
			});
		}
