	...
	function (
		...
		return function tableGridModelToCalsTable (tableGridModel, tgroupNode, blueprint, format) {
			...
			for (var rowIndex = 0, rowCount = tableGridModel.getHeight();
				rowIndex < rowCount;
				++rowIndex) {
				var firstCellInRow = tableGridModel.getCellAtCoordinates(rowIndex, 0);
				var row;
			...
			for (var unseenRowNodeId = unseenRowNodeIds.pop();
				unseenRowNodeId;
				unseenRowNodeId = unseenRowNodeIds.pop()) {
